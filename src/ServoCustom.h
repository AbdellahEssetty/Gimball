...
public:
    ...
    void setEasingFunction(int n)
    {
        switch(n)
        {
            case EasingFunctions::linear:
                easingFunction = std::bind(&ServoCustom::linear, this, std::placeholders::_1);
                break;
            case EasingFunctions::easeInQuadratic:
                easingFunction = std::bind(&ServoCustom::easeInOutCubic, this, std::placeholders::_1);
                break;
        }
    }
    ...
    void test()
    {
        if(easingFunction)
        {
            for(float x = 0.0; x < 1.8; x += 0.005)
            {
                servo.writeMicroseconds(600 + (int) 1000 * easingFunction(x));
                Serial.print(800 + (int) 1000 * easingFunction(x));
                Serial.print('\t');
                Serial.print(x);
                Serial.print('\t');
    ...
    // Write microsecond signal.
    void writeMicroseconds(int signal);
...
ServoCustom::ServoCustom(/* args */)
{
    Serial.println("Starting our custom servo");
}
...
void ServoCustom::attach(int pin) {
    servo.attach(pin);
}

void ServoCustom::write(int angle){
    servo.write(angle);
}

void ServoCustom::writeMicroseconds(int signal){
    servo.writeMicroseconds(signal);
}

// Modeled after the line y = x
float ServoCustom::linear(float x) {
  return x;
}

// Modeled after the parabola y = x^2
float ServoCustom::easeInQuadratic(float x) {
  return pow(x, 2);
}

// Modeled after the parabola y = -x^2 + 2x
float ServoCustom::easeOutQuadratic(float x) {
  return -(x * (x - 2));
}
...
float ServoCustom::easeInOutQuadratic(float x) {
  return (x < 0.5) ? (2 * pow(x, 2)) : ((-2 * pow(x, 2)) + (4 * x) - 1);
}

// Modeled after the cubic y = x^3
float ServoCustom::easeInCubic(float x) {
  return pow(x, 3);
}

// Modeled after the cubic y = (x - 1)^3 + 1
float ServoCustom::easeOutCubic(float x) {
  return pow(x - 1, 3) + 1;
}
...
float ServoCustom::easeInOutCubic(float x) {
  return (x < 0.5) ? (4 * pow(x, 3)) : (0.5 * pow(((2 * x) - 2), 3) + 1);
}

// Modeled after the quartic x^4
float ServoCustom::easeInQuartic(float x) {
  return pow(x, 4);
}

// Modeled after the quartic y = 1 - (x - 1)^4
float ServoCustom::easeOutQuartic(float x) {
...
float ServoCustom::easeInOutQuartic(float x) {
  return (x < 0.5) ? (8 * pow(x, 4)) : (-8 * pow(x - 1, 4) + 1);
}

// Modeled after the quintic y = x^5
float ServoCustom::easeInQuintic(float x) {
  return pow(x, 5);
}

// Modeled after the quintic y = (x - 1)^5 + 1
float ServoCustom::easeOutQuintic(float x) {
  return pow(x - 1, 5) + 1;
}
...
float ServoCustom::easeInOutQuintic(float x) {
  return (x < 0.5) ? (16 * pow(x, 5)) : (0.5 * pow((2 * x) - 2, 5) + 1);
}

// Modeled after quarter-cycle of sine wave
float ServoCustom::easeInSine(float x) {
  return sin((x - 1) * M_PI_2) + 1;
}

// Modeled after quarter-cycle of sine wave (different phase)
float ServoCustom::easeOutSine(float x) {
  return sin(x * M_PI_2);
}

// Modeled after half sine wave
float ServoCustom::easeInOutSineEaseInOut(float x) {
  return 0.5 * (1 - cos(x * M_PI));
}

// Modeled after shifted quadrant IV of unit circle
float ServoCustom::easeInCircular(float x) {
  return 1 - sqrt(1 - pow(x, 2));
}

// Modeled after shifted quadrant II of unit circle
float ServoCustom::easeOutCircular(float x) {
  return sqrt((2 - x) * x);
}
...
float ServoCustom::easeInOutCircular(float x) {
  return (x < 0.5) ? (0.5 * (1 - sqrt(1 - 4 * pow(x, 2)))) : (0.5 * (sqrt(-((2 * x) - 3) * ((2 * x) - 1)) + 1));
}

// Modeled after the exponential function y = 2^(10(x - 1))
float ServoCustom::easeInExponential(float x) {
  return (x == 0.0) ? x : pow(2, 10 * (x - 1));
}

// Modeled after the exponential function y = -2^(-10x) + 1
float ServoCustom::easeOutExponential(float x) {
...
float ServoCustom::easeInOutExponential(float x) {
...
float ServoCustom::easeInElastic(float x) {
  return sin(13 * M_PI_2 * x) * pow(2, 10 * (x - 1));
}

// Modeled after the damped sine wave y = sin(-13pi/2*(x + 1))*pow(2, -10x) + 1
float ServoCustom::easeOutElastic(float x) {
...
float ServoCustom::easeInOutElastic(float x) {
  return (x < 0.5) ? 
...
float ServoCustom::easeInBack(float x) {
  return pow(x, 3) - x * sin(x * M_PI);
}

// Modeled after overshooting cubic y = 1-((1-x)^3-(1-x)*sin((1-x)*pi))
float ServoCustom::easeOutBack(float x) {
...
float ServoCustom::easeInOutBack(float x) {
  return (x < 0.5) ? 
    (0.5 * (pow(2 * x, 3) - 2 * x * sin(2 * x * M_PI)))
    : (0.5 * (1 - (pow((1 - (2 * x - 1)), 3) - (1 - (2 * x - 1)) * sin((1 - (2 * x - 1)) * M_PI))) + 0.5);
}

float ServoCustom::easeInBounce(float x) {
  return 1 - easeOutBounce(1 - x);
}

float ServoCustom::easeOutBounce(float x) {
  if (x < 1 / 2.75) {
...
float ServoCustom::easeInOutBounce(float x) {
  return (x < 0.5) ? (0.5 * ServoCustom::easeInBounce(x * 2)) : (0.5 * ServoCustom::easeOutBounce(x * 2 - 1) + 0.5);
}